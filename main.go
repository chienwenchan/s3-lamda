...
func HandleLambdaEvent(ctx context.Context, event EvEnt) (string, error) {
	...
	for i := 0; i < step; i++ {
		...
		for j, s := range tmpS {
			...
			go func(sp Split, sw *sync.WaitGroup, svc1 *s3.S3, start, index int) {
				...
				if min+j+1 == len(config.Split)-1 {
					out, err := os.Create(basePath + "/" + config.Split[len(config.Split)-1].Key)
					if err != nil {
						log.Printf("错误x1:%s", err.Error())
						sw.Done()
						return
					}
					_, _ = io.Copy(out, split.Body)
	...
	if len(datas) != len(config.Split)-1 {
